...
const Analytics: React.FC<AnalyticsProps> = ({ darkMode }) => {
  ...
  const filteredTransactions = useMemo(() => {
    return mockTransactions.filter((transaction) => {
      ...
      const barberMatch =
        !selectedBarber || transaction.barberId === selectedBarber;
      const serviceMatch =
        !selectedService || transaction.services.includes(selectedService);
  ...
  const barberPerformance = useMemo(() => {
    const performance = barbers.map((barber) => {
      const barberTransactions = filteredTransactions.filter(
        (t) => t.barberId === barber.id
      );
      const totalRevenue = barberTransactions.reduce(
        (sum, t) => sum + t.finalAmount,
        0
      );
      const totalServices = barberTransactions.reduce(
  ...
  const dailyRevenue = useMemo(() => {
    ...
    filteredTransactions.forEach((transaction) => {
      ...
      dailyStats[dateKey] =
        (dailyStats[dateKey] || 0) + transaction.finalAmount;
    });
